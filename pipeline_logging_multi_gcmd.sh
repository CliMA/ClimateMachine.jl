...
while read -u3 -r line
  do
    ...
    mpiexec julia --project=$CLIMA_HOME $CLIMA_RUNFILE --diagnostics 0.5shours --monitor-courant-numbers 0.5shours --output-dir $CLIMA_NETCDF --checkpoint-at-end --checkpoint-dir $CLIMA_RESTART --sim-time=2592000
    sleep 2
    
    # move runfile and .out file
    mv $CLIMA_RUNFILE $CLIMA_LOG;
    cp model_log_err.out $CLIMA_LOG/$(basename $CLIMA_RUNFILE)'_out'
