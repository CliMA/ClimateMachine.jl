...
export NumericalFluxNonDiffusive,
...
function numerical_flux_nondiffusive!(
    ...
    update_penalty!(nf, bl, n, λ, Vars{S}(λΔQ), state⁻, aux⁻, state⁺, aux⁺, t)
...
function divergence_boundary_penalty!(
    nf::CentralDivPenalty,
    bl::BalanceLaw,
    div_penalty::Vars{GL},
    n::SVector,
    grad⁻::Grad{GL},
...
function numerical_flux_hyperdiffusive!(
...
function numerical_boundary_flux_hyperdiffusive!(
    nf::CentralHyperDiffusiveFlux,
    bl::BalanceLaw,
    hyperdiff::Vars{HD},
    n::SVector,
    lap⁻::Vars{GL},
...
numerical_boundary_flux_diffusive!(
    ...
    diff1⁻::Vars{D},
    aux1⁻::Vars{A},
) where {S, D, HD, A} = normal_boundary_flux_diffusive!(
    nf,
    bl,
    fluxᵀn,
    n⁻,
