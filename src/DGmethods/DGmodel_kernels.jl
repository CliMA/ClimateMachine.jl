                ...
                @loop for i in (1:Nq; threadIdx().x)
                    ...
                    @unroll for s in 1:nstate
                        @unroll for n in 1:Nq
                            # ξ1-grid lines
                            l_rhs[s, i, j, k] +=
                                MI * s_D[n, i] * s_F[1, n, j, k, s]

                            # ξ2-grid lines
                            if dim == 3 ||
                               (dim == 2 && direction == EveryDirection)
                                l_rhs[s, i, j, k] +=
                                    MI * s_D[n, j] * s_F[2, i, n, k, s]
                            end

                            # ξ3-grid lines
                            if dim == 3 && direction == EveryDirection
                                l_rhs[s, i, j, k] +=
                                    MI * s_D[n, k] * s_F[3, i, j, n, s]
                            end
                        end
                    end
                end
                    ...
                    @unroll for s in 1:nstate
                        @unroll for n in 1:Nq
                            if dim == 2
                                Dnj = s_D[n, j]
                                l_rhs[s, i, j, k] +=
                                    MI * Dnj * s_F[3, i, n, k, s]
                            else
                                Dnk = s_D[n, k]
                                l_rhs[s, i, j, k] +=
                                    MI * Dnk * s_F[3, i, j, n, s]
                            end
                        end
                    end
                end
...
function knl_copy_stack_field_down!(
    ::Val{dim},
    ::Val{N},
    ::Val{nvertelem},
    auxstate,
        ...
        @loop for k in (1:Nqk; threadIdx().z)
            @loop for j in (1:Nq; threadIdx().y)
                @loop for i in (1:Nq; threadIdx().x)
                    ijk = i + Nq * ((j - 1) + Nq * (k - 1))
                    if dim == 2
                        ξvx1, ξvx2, ξvx3 = vgeo[ijk, _ξ2x1, e],
                        vgeo[ijk, _ξ2x2, e],
                        vgeo[ijk, _ξ2x3, e]
                    else
                        ξvx1, ξvx2, ξvx3 = vgeo[ijk, _ξ3x1, e],
                        vgeo[ijk, _ξ3x2, e],
                        vgeo[ijk, _ξ3x3, e]
                    end
                    @unroll for s in 1:ngradlapstate
