...
@kernel function volumerhs!(bl::BalanceLaw, ::Val{dim}, ::Val{polyorder}, ::direction,
                            ...
                            @Const(ω), @Const(D), ::Val{nvgeo}, ::Val{increment}) where {dim, polyorder, direction, nvgeo, increment}
  @uniform begin
    N = polyorder
    FT = eltype(Q)
  ...
  @inbounds begin
    ...
    @unroll for k in 1:Nqk
      ...
      @unroll for s = 1:nstate
        @unroll for n = 1:Nq
          # ξ1-grid lines
          l_rhs[k, s] += MI * D[n, i] * s_F[1, n, j, s]

          # ξ2-grid lines
          if dim == 3 || (dim == 2 && direction == EveryDirection)
            l_rhs[k, s] += MI * D[n, j] * s_F[2, i, n, s]
          end
        end
      end
    end
...
@kernel function volumerhs!(bl::BalanceLaw, ::Val{dim}, ::Val{polyorder}, ::VerticalDirection,
                            rhs, @Const(Q), @Const(Qvisc), @Const(Qhypervisc_grad),
                            @Const(auxstate), @Const(vgeo), t,
                            @Const(ω), @Const(D), ::Val{nvgeo}, ::Val{increment}) where {dim, polyorder, nvgeo, increment}
  @uniform begin
    N = polyorder
    FT = eltype(Q)
    nstate = num_state(bl,FT)
    ...
    l_aux = MArray{Tuple{nauxstate}, FT}(undef)
    l_F = MArray{Tuple{3, nstate}, FT}(undef)
    l_F2 = MArray{Tuple{3, nstate}, FT}(undef)

    _ξx1 = dim == 2 ? _ξ2x1 : _ξ3x1
    _ξx2 = dim == 2 ? _ξ2x2 : _ξ3x2
    _ξx3 = dim == 2 ? _ξ2x3 : _ξ3x3
  end
  ...
  @inbounds begin
    ...
    @unroll for k in 1:Nqk
      ...
      # Build "inside metrics" flux
      @unroll for s = 1:nstate
        F1, F2, F3 = l_F2[1, s], l_F2[2, s], l_F2[3, s]
        Fv =  M * (ξx1 * F1 + ξx2 * F2 + ξx3 * F3) 
        if dim == 2
          s_F[i,j,s] = Fv
        else
          l_F2[1, s] = Fv
