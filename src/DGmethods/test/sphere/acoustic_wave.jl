...
#--------------------------------#
#Can be run with:
# mpirun -n 1 julia --project=@. acoustic_wave.jl
# mpirun -n 2 julia --project=@. acoustic_wave.jl
#--------------------------------#
#--------------------------------#
...
# Euler flux function
euler_flux!(F, Q, QV, aux, t) = euler_flux!(F, Q, QV, aux, t, preflux(Q,QV,aux,t)...)
@inline function euler_flux!(F,Q,QV,aux,t,P,u,v,w,ρinv)
  @inbounds begin
...
#    nsimstep=10
    ...
    # Set up Maxs/Mins callback
    cbmaxs = GenericCallbacks.EveryXSimulationSteps(nsimstep) do
        @info @sprintf """Max Values
            ρ_max = %.16e
            U_max = %.16e
            V_max = %.16e
            W_max = %.16e
            E_max = %.16e""" global_max(Q,_ρ) global_max(Q,_U) global_max(Q,_V) global_max(Q,_W) global_max(Q,_E)
    end
    cbmins = GenericCallbacks.EveryXSimulationSteps(nsimstep) do
        @info @sprintf """Min Values
            ρ_min = %.16e
            U_min = %.16e
            V_min = %.16e
            W_min = %.16e
            E_min = %.16e""" global_min(Q,_ρ) global_min(Q,_U) global_min(Q,_V) global_min(Q,_W) global_min(Q,_E)
    end
    ...
    cbvtk = GenericCallbacks.EveryXSimulationSteps(nsimstepvtk) do (init=false)
        outprefix = @sprintf("vtk/acoustic_wave_%dD_mpirank%04d_step%04d",
