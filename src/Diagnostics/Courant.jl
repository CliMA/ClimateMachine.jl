...
""" dt_exp dt_imp cfl_h cfl_v cflu_h cflu_v cfldiff_h cfldiff_v
    ...
    return dt_exp, dt_imp
   
end # function gather_Courant

function gather_dt_given_Courant(mpicomm, dg, Q, xmax, ymax, Dx, Dy, Dz, Courant)
    ...
    max_sound_speed = MPI.Allreduce(ss,MPI.MAX,mpicomm)
    dt_exp = Courant * min(Dx, Dy, Dz) / max_sound_speed
    dt_imp = Courant * max(Dx, Dy, Dz) / max_sound_speed
