...
Base.collect(element::RectangularElement) = RectangularElement(collect(data), collect(x), collect(y), collect(z))
...
Base.show(io::IO, elem::RectangularElement{D}) where D =
    print(io, "RectangularElement{$(D.name.wrapper)} with ",
...
function x_glue(west::RectangularElement, east::RectangularElement)
    ...
    data = cat(west.data[1:end-1, :, :], contact, east.data[2:end, :, :], dims=1)
...
function y_glue(south::RectangularElement, north::RectangularElement)
    ...
    data = cat(south.data[:, 1:end-1, :], contact, north.data[:, 2:end, :], dims=2)
...
function z_glue(bottom::RectangularElement, top::RectangularElement)
    ...
    data = cat(bottom.data[:, :, 1:end-1], contact, top.data[:, :, 2:end], dims=3)
