...
vars_state(cm::Continuity3dModel, FT) = vars_state(cm.ocean, FT)

# Continuity3dModel is used to compute the horizontal divergence of u

vars_aux(cm::Continuity3dModel, T)  = @vars()
vars_gradient(cm::Continuity3dModel, T)  = @vars()
vars_diffusive(cm::Continuity3dModel, T)  = @vars()
vars_integrals(cm::Continuity3dModel, T)  = @vars()
init_aux!(cm::Continuity3dModel, _...) = nothing
init_state!(cm::Continuity3dModel, _...) = nothing
@inline flux_diffusive!(cm::Continuity3dModel, _...) = nothing
@inline source!(cm::Continuity3dModel, _...) = nothing
@inline update_penalty!(
         ::Rusanov,
         ::Continuity3dModel, _...) = nothing

# This allows the balance law framework to compute the horizontal gradient of u
# (which will be stored back in the field θ)
@inline function flux_nondiffusive!(m::Continuity3dModel, flux::Grad,
