...
function flux_first_order!(
    ...
    flux.energy.ρe = Σfluxes(eq_tends(Energy(), atmos, tend), atmos, args)
end

function flux_first_order!(energy::θModel, atmos::AtmosModel, flux::Grad, args)
    tend = Flux{FirstOrder}()
...
function flux_second_order!(energy::θModel, flux::Grad, atmos::AtmosModel, args)
    tend = Flux{SecondOrder}()
    flux.energy.ρθ_liq_ice =
        Σfluxes(eq_tends(ρθ_liq_ice(), atmos, tend), atmos, args)
end
...
function source!(energy::EnergyModel, source::Vars, atmos::AtmosModel, args)
    tend = Source()
    source.energy.ρe = Σsources(eq_tends(Energy(), atmos, tend), atmos, args)
end
