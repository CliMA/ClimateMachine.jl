...
function dynamic_viscosity_tensor(m::ConstantViscosityWithDivergence, atmos::AtmosModel, S,
...
vars_aux(::SmagorinskyLilly,FT) = @vars(Δ::FT,ρν::FT,BR::FT,Freq::FT)
vars_gradient(::SmagorinskyLilly,FT) = @vars(θ_v::FT)
vars_diffusive(::SmagorinskyLilly,FT) = @vars(BR::FT,Freq::FT)
...
function atmos_nodal_update_aux!(::SmagorinskyLilly, ::AtmosModel, state::Vars, aux::Vars, diffusive::Vars, t::Real)
  aux.turbulence.ρν = diffusive.ρν
  aux.turbulence.BR = diffusive.turbulence.BR
  aux.turbulence.Freq = diffusive.turbulence.Freq
end
...
"""
function squared_buoyancy_correction(normS, atmos::AtmosModel, ∇transform::Grad, aux::Vars)
  FT = eltype(normS)
...
function strain_rate_magnitude(S::SHermitianCompact{3,FT,6}) where {FT}
  return @inbounds sqrt(2*S[1,1]^2 + 4*S[2,1]^2 + 4*S[3,1]^2 + 2*S[2,2]^2 + 4*S[3,2]^2 + 2*S[3,3]^2)
end

function dynamic_viscosity_tensor(m::SmagorinskyLilly, atmos::AtmosModel, S, state::Vars, diffusive::Vars, ∇transform::Grad, aux::Vars, t::Real)
  # strain rate tensor norm
  # Notation: normS ≡ norm2S = √(2S:S)
  # ρν = (Cₛ * Δ * f_b)² * √(2S:S)
  FT = eltype(state)
  @inbounds normS = strain_rate_magnitude(S)
  f_b² = squared_buoyancy_correction(normS, atmos, ∇transform, aux)
...
function atmos_nodal_update_aux!(::Vreman, ::AtmosModel, state::Vars, aux::Vars, diffusive::Vars, t::Real)
  aux.turbulence.ρν = diffusive.ρν
  aux.turbulence.BR = diffusive.turbulence.BR
end
...
function dynamic_viscosity_tensor(m::Vreman, atmos::AtmosModel, S, state::Vars, diffusive::Vars, ∇transform::Grad, aux::Vars, t::Real)
  FT = eltype(state)
  ∇u = ∇transform.u
  αijαij = sum(∇u .^ 2)
  @inbounds normS = strain_rate_magnitude(S)
...
vars_aux(::AnisoMinDiss,FT) = @vars(Δ::FT,ρν::FT,BR::FT,Freq::FT)
...
function atmos_nodal_update_aux!(::AnisoMinDiss, ::AtmosModel, state::Vars, aux::Vars, diffusive::Vars, t::Real)
  aux.turbulence.ρν = diffusive.ρν
  aux.turbulence.BR = diffusive.turbulence.BR
end
...
function dynamic_viscosity_tensor(m::AnisoMinDiss, atmos::AtmosModel, S, state::Vars, diffusive::Vars, ∇transform::Grad, aux::Vars, t::Real)
  FT = eltype(state)
  ∇u = ∇transform.u
  αijαij = dot(∇u,∇u)
