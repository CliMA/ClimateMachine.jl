...
"""
    BulkFormulaEnergySpatiallyVarying(fn) :: EnergyBC
...
Input is either a zonally-symmetric idealised SST field or observations.  
"""
struct BulkFormulaEnergySpatiallyVarying{FNX, FNT} <: EnergyBC
    fn_C_h::FNX
    fn_T_sfc::FNT
end
function atmos_energy_boundary_state!(
    nf,
    bc_energy::BulkFormulaEnergySpatiallyVarying,
    atmos,
    args...,
) end
...
function atmos_energy_normal_boundary_flux_second_order!(
    ...
    φ = latitude( atmos.orientation, aux_int⁻)

    u_int⁻ = state_int⁻.ρu / state_int⁻.ρ
    u_int⁻_tan = projection_tangential(atmos, aux_int⁻, u_int⁻)
    normu_int⁻_tan = norm(u_int⁻_tan)
    C_h = bc_energy.fn_C_h(state_sfc⁻, aux_sfc⁻, t, normu_int⁻_tan)
    T_sfc_min = bc_energy.fn_T_sfc(state_sfc⁻, aux_sfc⁻, t)
    #q_tot_sfc = bc_energy.fn_q_tot_sfc(state_sfc⁻, aux_sfc⁻, t)
    T_sfc, q_tot_sfc = get_spatial_SST_idealized( atmos, aux_int⁻, state_int⁻, T_sfc_min )
