...
function integrate_aux!(m::StevensRadiation, integrand::Vars, flux::Grad, state::Vars, diffusive::Vars, aux::Vars, t::Real)
  ...
  x,y,z = aux.coord
  κ = T(85)
  integrand.integrand_1 = state.ρ * κ * aux.moisture.q_liq
end

function flux!(m::StevensRadiation, flux::Grad, state::Vars, diffusive::Vars, aux::Vars, t::Real)
  T = eltype(flux)
  z_i = T(840) # Constant inversion height zᵢ 
  Δz_i = max(aux.z - z_i, -zero(T))
  # Constants
  α_z = T(1) 
  ρ_i = T(1.22) # Surface Density
  D_subsidence = T(3.75e-6) # Large-scale Divergence
