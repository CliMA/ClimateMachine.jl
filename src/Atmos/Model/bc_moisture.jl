...
"""
    BulkFormulaMoistureSpatiallyVarying(fn) :: MoistureBC

Calculate the net inward moisture flux across the boundary using
the bulk formula.
The drag coefficient is `C_q = fn_C_q(state, aux, t, normu_int_tan)`.
The surface q_tot is calculated from the SSTs
Return the flux (in kg m^-2 s^-1).


"""
struct BulkFormulaMoistureSpatiallyVarying{FNX, FNT} <: MoistureBC
    fn_C_q::FNX
    fn_T_sfc::FNT
end
function atmos_moisture_boundary_state!(
    nf,
    bc_moisture::BulkFormulaMoistureSpatiallyVarying,
    atmos,
    args...,
) end
...
function atmos_moisture_normal_boundary_flux_second_order!(
    ...
    T_sfc_min = bc_moisture.fn_T_sfc(state_sfc⁻, aux_sfc⁻, t)
