...
"""
@inline over_elems_real(grid::Grid) =
    (1 + grid.n_ghost):(grid.n_elem - grid.n_ghost)
...
Get the first element index in the ghost domain
"""
@inline boundary_points(grid::Grid, b::ZBoundary) =
    first_ghost(grid, b), first_interior(grid, b), second_interior(grid, b)
...
"""
@inline over_elems_ghost(grid::Grid) =
    setdiff(over_elems(grid), over_elems_real(grid))
...
@inline over_elems_ghost(grid::Grid, ::Zmax) =
    (grid.n_elem - grid.n_ghost + 1):(grid.n_elem)
