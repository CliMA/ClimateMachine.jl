...
function TridiagonalMatrix(
    ...
    a_ = [
        -Δt *
        grid.Δzi2 *
        (tmp[ρ, Dual(k)] .* q[a, Dual(k), i] .* tmp[K, Dual(k), i])[2]
        for k in over_elems_real(grid)[1:(end - 1)]
    ]
    b_ = [
        1 +
        Δt *
        grid.Δzi2 *
        (sum((tmp[ρ, Dual(k)] .* q[a, Dual(k), i] .* tmp[K, Dual(k), i])))
        for k in over_elems_real(grid)
    ]
    c_ = [
        -Δt *
        grid.Δzi2 *
        (tmp[ρ, Dual(k)] .* q[a, Dual(k), i] .* tmp[K, Dual(k), i])[1]
        for k in over_elems_real(grid)[2:end]
    ]
