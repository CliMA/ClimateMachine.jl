...
function alpha_sic(am::aerosol_model)
    return ntuple(length(am.modes)) do i 
        ...
        exp1 = (grav*molmass_water*L) / (cp_d*R*T^2)
        exp2 = (grav*avg_molar_mass)/(R*T)
        exp1-exp2
    end
end
...
function gamma_sic(am::aerosol_model, P_sat)
    return ntuple(length(am.modes)) do i 
        ...
        avg_molar_mass = numerator/denominator
        exp1 = (R*T)/(P_sat*molmass_water)
        exp2 = (molmass_water*L^2)/(cp_d*P*avg_molar_mass*T)
        exp1+exp2 
    end
end
