...
q_vap_saturation_from_pressure(T::FT, ρ::FT, p_v_sat::FT) where {FT<:Real} =
  ...
  # p_v_sat / (ρ * FT(R_v) * T) # TODO: swap to convert to master
...
function saturation_adjustment_SecantMethod(e_int::FT, ρ::FT, q_tot::FT, tol::FT, maxiter::Int) where {FT<:Real}
  ...
  unsaturated = q_tot <= q_v_sat
  if unsaturated
  # if unsaturated && T_1 > FT(T_min) # TODO: swap to convert to master
    return T_1
  else
    ...
    # T_2 = bound_upper_temperature(T_1, T_2) # TODO: Add to convert to master
    sol = find_zero(
      T -> internal_energy_sat(T, ρ, q_tot) - e_int,
...
function saturation_adjustment_q_tot_θ_liq_ice(θ_liq_ice::FT, ρ::FT, q_tot::FT, tol::FT, maxiter::Int) where {FT<:Real}
  ...
  # T_1 = max(FT(T_min), air_temperature_from_liquid_ice_pottemp(θ_liq_ice, ρ, PhasePartition(q_tot))) # Assume all vapor  # TODO: swap to convert to master
  q_v_sat = q_vap_saturation(T_1, ρ)
  unsaturated = q_tot <= q_v_sat
  # if unsaturated # TODO: swap to convert to master
  if unsaturated && T_1 > FT(T_min)
    return T_1
  else
...
function air_temperature_from_liquid_ice_pottemp(θ_liq_ice::FT, ρ::FT,
  ...
  return T_u + T_1
  # return T_u + T_1 + T_2 # TODO: swap to convert to master
end
