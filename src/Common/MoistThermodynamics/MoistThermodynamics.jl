...
"""
air_temperature_from_liquid_ice_pottemp_old(θ_liq_ice::FT, p::FT, q::PhasePartition{FT}=q_pt_0(FT), param_set::APS{FT}=MTPS{FT}()) where {FT<:Real} =
  θ_liq_ice*exner_given_pressure(p, q, param_set) + (FT(LH_v0)*q.liq + FT(LH_s0)*q.ice) / cp_m(q, param_set)

air_temperature_from_liquid_ice_pottemp_old(θ_liq_ice::FT, p::FT, param_set::APS{FT}) where {FT<:Real} =
  air_temperature_from_liquid_ice_pottemp_old(θ_liq_ice, p, q_pt_0(FT), param_set)
...
"""
function saturation_adjustment_q_tot_θ_liq_ice_old(θ_liq_ice::FT,
                                                   q_tot::FT,
                                                   ρ::FT,
                                                   p::FT,
                                                   param_set::APS{FT}) where {FT<:Real}
  T_1 = air_temperature_from_liquid_ice_pottemp_old(θ_liq_ice, p, param_set) # Assume all vapor
  q_v_sat = q_vap_saturation(T_1, ρ, param_set)
  T_sol = T_1
  if q_tot <= q_v_sat # If not saturated
  else  # If saturated, iterate
    q_pt_ice = PhasePartition(q_tot, FT(0), q_tot) # Assume all ice
    T_2 = air_temperature_from_liquid_ice_pottemp_old(θ_liq_ice, p, q_pt_ice, param_set)
    function eos(T)
      q_vap_sat = q_vap_saturation(T, ρ, param_set)
      q_pt = PhasePartition(q_vap_sat)
      θ_dry = dry_pottemp_given_pressure(T, p, q_pt, param_set)
      exp_fact = exp(-latent_heat_vapor(T, param_set)/(T*cp_d)*(q_tot - q_vap_sat)/(1.0-q_tot))
      return θ_liq_ice - θ_dry * exp_fact
    end

    # TODO/TOFIX: SCAMPY implementation
    tol, maxiter = FT(1e-3), 10
    sol = find_zero(T -> eos(T), T_1, T_2, SecantMethod(), CompactSolution(), tol, maxiter)
    T_sol = sol.root
  end
  return T_sol
end
