...
function saturation_adjustment_q_tot_θ_liq_ice_old(θ_liq_ice::FT, q_tot::FT, ρ::FT, p::FT) where {FT<:Real}
  ...
  if q_tot <= q_v_sat # If not saturated
    ...
    T_2 = air_temperature_from_liquid_ice_pottemp_old(θ_liq_ice, p, PhasePartition(q_tot, FT(0), q_tot)) # Assume all ice
    function eos(T)
      q_vap_sat = q_vap_saturation(T, ρ)
      q_pt = PhasePartition(q_vap_sat)
      return θ_liq_ice - dry_pottemp_given_pressure(T, p, q_pt) * exp(-latent_heat_vapor(T)/(T*cp_d)*(q_tot - q_vap_sat)/(1.0-q_tot))
    end
