...
function PhaseEquil(e_int::FT,
                    ...
                    tol::FT=FT(1e-3),
                    # maxiter::Int=3, # TODO: swap to convert to master
                    maxiter::Int=10,
                    sat_adjust::F=saturation_adjustment_SecantMethod) where {FT<:Real,F}
                    # sat_adjust::F=saturation_adjustment) where {FT<:Real,F} # TODO: swap to convert to master
...
function LiquidIcePotTempSHumEquil(θ_liq_ice::FT,
                                   ...
                                   # tol::FT=FT(1e-1), # TODO: swap to convert to master
                                   tol::FT=FT(1e-5),
                                   # maxiter::Int=10 # TODO: swap to convert to master
                                   maxiter::Int=40
                                   ) where {FT<:Real}
...
function LiquidIcePotTempSHumEquil_given_pressure(θ_liq_ice::FT,
                                                  ...
                                                  # maxiter::Int=30) where {FT<:Real} # TODO:swap to convert to master
