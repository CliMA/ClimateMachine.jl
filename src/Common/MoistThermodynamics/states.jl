...
Constructs a [`PhaseEquil`](@ref) thermodynamic state from:
 ...
 - `p` - pressure
"""
function LiquidIcePotTempSHumEquil_old(θ_liq_ice::FT, q_tot::FT, ρ::FT, p::FT, param_set::PS = MTPS{FT}()) where {FT<:Real,PS}
    T = saturation_adjustment_q_tot_θ_liq_ice_old(θ_liq_ice, q_tot, ρ, p, param_set)
