...
"""
air_pressure_given_θ(θ::FT, Φ::FT, ::DryAdiabaticProcess, param_set::APS{FT}=MTPS{FT}()) where {FT} =
...
The air pressure for an isentropic process, where
 ...
 - `p∞` ambient pressure
"""
air_pressure(T::FT, T∞::FT, p∞::FT, ::DryAdiabaticProcess, param_set::APS{FT}=MTPS{FT}()) where {FT} =
...
The air temperature for an isentropic process, where
 ...
 - `θ` potential temperature
"""
air_temperature(p::FT, θ::FT, ::DryAdiabaticProcess, param_set::APS{FT}=MTPS{FT}()) where {FT} =
