...
struct Configuration{DeviceArray, HostArray} #<: AD.AbstractSpaceConfiguration
  ...
  function Configuration(params::Parameters, mpicomm)
    ...
    mpirank == 0 && @debug "create grad storage..."
    ngrad = _nstategrad + 3*params.nmoist
    grad = zeros(DFloat, size(vgeo,1), ngrad, size(vgeo,3))
    ...
    sendgrad = zeros(DFloat, (N+1)^dim, ngrad, length(mesh.sendelems))
    recvgrad = zeros(DFloat, (N+1)^dim, ngrad, length(mesh.ghostelems))
    ...
    new{DeviceArray, HostArray}(mpicomm, mesh, DeviceArray(vgeo),
                                ...
                                DeviceArray(recvQ),
                                DeviceArray(grad), sendgrad, recvgrad,
                                DeviceArray(sendgrad),
                                DeviceArray(recvgrad),
                                DeviceArray(D))
  end
end
# }}}
...
function AD.rhs!(rhs::DeviceArray,
  ...
  host_recvgrad = config.host_recvgrad
  host_sendgrad = config.host_sendgrad
  device_recvgrad = config.device_recvgrad
  device_sendgrad = config.device_sendgrad
  ...
  DFloat = eltype(Q)
  viscosity::DFloat = params.viscosity
  ...
  volumegrad!(Val(dim), Val(N), Val(nmoist), Val(ntrace), grad, Q, vgeo,
              gravity, Dmat, mesh.realelems)
  ...
  # post MPI receives
  for n = 1:nnabr
    recvreq[n] = MPI.Irecv!((@view host_recvgrad[:, :, mesh.nabrtorecv[n]]),
                            mesh.nabrtorank[n], 777, mpicomm)
  end

  # face grad computation
  facegrad!(Val(dim), Val(N), Val(nmoist), Val(ntrace), grad, Q, vgeo,
            sgeo, gravity, mesh.realelems, vmapM, vmapP, elemtobndy)
  ...
  # post MPI sends
  for n = 1:nnabr
    sendreq[n] = MPI.Isend((@view host_sendgrad[:, :, mesh.nabrtosend[n]]),
                           mesh.nabrtorank[n], 777, mpicomm)
  end

  # volume RHS computation
  volumerhs!(Val(dim), Val(N), Val(nmoist), Val(ntrace), rhs, Q, grad, vgeo,
             gravity, viscosity, Dmat, mesh.realelems)
  ...
  facerhs!(Val(dim), Val(N), Val(nmoist), Val(ntrace), rhs, Q, grad, vgeo,
           sgeo, gravity, viscosity, mesh.realelems, vmapM, vmapP, elemtobndy)
end
# }}}
...
function transferrecvbuf!(device_recvbuf::Array, host_recvbuf, buf, nrealelem)
