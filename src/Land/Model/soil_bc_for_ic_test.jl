...
function execute_based_on_boundaries!(
    ...
    if bctype == 2
        if bc.surface_flux != nothing
            ...
            #@printf("%le %le %le %le\n", t, S, matric_potential(soil.water.hydraulics,S),(FT(1.0)-matric_potential(soil.water.hydraulics, S)/Δz))
            diff⁺.soil.water.K∇h = n̂ * -(bc.surface_flux(aux⁻, t))*land.soil.param_functions.Ksat#*(eltype(state⁻)(1.0)-matric_potential(soil.water.hydraulics,S)/Δz) #(-bc.surface_flux(aux⁻, t))
        else
            nothing
        end
    elseif bctype == 1
