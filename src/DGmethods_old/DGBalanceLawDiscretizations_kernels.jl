...
function volumerhs!(
    ...
    elems,
    increment,
) where {dim, N, nstate, nviscstate, nauxstate}
    FT = eltype(Q)
                ...
                @loop for i in (1:Nq; threadIdx().x)
                    @unroll for n in 1:Nq
                        @unroll for s in 1:nstate
                            ...
                            # ξ1-grid lines
                            l_rhs[s, i, j, k] +=
                                l_ξ1x1[i, j, k] * Dni * s_F[1, n, j, k, s]
                            l_rhs[s, i, j, k] +=
                                l_ξ1x2[i, j, k] * Dni * s_F[2, n, j, k, s]
                            l_rhs[s, i, j, k] +=
                                l_ξ1x3[i, j, k] * Dni * s_F[3, n, j, k, s]

                            # ξ2-grid lines
                            l_rhs[s, i, j, k] +=
                                l_ξ2x1[i, j, k] * Dnj * s_F[1, i, n, k, s]
                            l_rhs[s, i, j, k] +=
                                l_ξ2x2[i, j, k] * Dnj * s_F[2, i, n, k, s]
                            l_rhs[s, i, j, k] +=
                                l_ξ2x3[i, j, k] * Dnj * s_F[3, i, n, k, s]

                            # ξ3-grid lines
                            if Nqk > 1
                                l_rhs[s, i, j, k] +=
                                    l_ξ3x1[i, j, k] * Dnk * s_F[1, i, j, n, s]
                                l_rhs[s, i, j, k] +=
                                    l_ξ3x2[i, j, k] * Dnk * s_F[2, i, j, n, s]
                                l_rhs[s, i, j, k] +=
                                    l_ξ3x3[i, j, k] * Dnk * s_F[3, i, j, n, s]
                            end
                        end
                    end
                end
    ...
    @inbounds @loop for e in (elems; blockIdx().x)
        for f in 1:nface
            @loop for n in (1:Nfp; threadIdx().x)
                nM =
                    (sgeo[_n1, n, f, e], sgeo[_n2, n, f, e], sgeo[_n3, n, f, e])
...
function volumeviscterms!(
    ...
    D,
    elems,
) where {dim, N, ngradstate, nviscstate, nstate, nauxstate}
    FT = eltype(Q)
    ...
    @inbounds @loop for e in (elems; blockIdx().x)
        ...
        @loop for k in (1:Nqk; threadIdx().z)
            @loop for j in (1:Nq; threadIdx().y)
                @loop for i in (1:Nq; threadIdx().x)
                    ijk = i + Nq * ((j - 1) + Nq * (k - 1))
                    ξ1x1, ξ1x2, ξ1x3 = vgeo[ijk, _ξ1x1, e],
                    vgeo[ijk, _ξ1x2, e],
                    vgeo[ijk, _ξ1x3, e]
        ...
        for d in 1:3
            @loop for k in (1:Nqk; threadIdx().z)
                @loop for j in (1:Nq; threadIdx().y)
                    @loop for i in (1:Nq; threadIdx().x)
                        s_f[1, i, j, k] =
                            l_J[i, j, k] * l_ξ1d[d, i, j, k] * l_f[i, j, k]
                        s_f[2, i, j, k] =
                            l_J[i, j, k] * l_ξ2d[d, i, j, k] * l_f[i, j, k]
                        s_f[3, i, j, k] =
                            l_J[i, j, k] * l_ξ3d[d, i, j, k] * l_f[i, j, k]
                    end
                end
            end
            @synchronize
                ...
                @loop for j in (1:Nq; threadIdx().y)
                    @loop for i in (1:Nq; threadIdx().x)
                        ...
                        @unroll for n in 1:Nq
                            ...
                            Nqk > 1 &&
                            (l_fd[d, i, j, k] += JI * Dkn * s_f[3, i, j, n])
                        end
                    end
                end
            end
...
function knl_dof_iteration!(
    ...
    auxstate,
    elems,
) where {dim, N, nRstate, nstate, nviscstate, nauxstate}
    FT = eltype(R)
...
See [`DGBalanceLaw`](@ref) for usage.
"""
function knl_indefinite_stack_integral!(
    ::Val{dim},
    ::Val{N},
    ::Val{nstate},
    ::Val{nauxstate},
    ...
    elems,
    ::Val{outstate},
) where {dim, N, nstate, nauxstate, outstate, nvertelem}
    FT = eltype(Q)
...
function knl_reverse_indefinite_stack_integral!(
    ::Val{dim},
    ::Val{N},
    ::Val{nvertelem},
    P,
    ...
    ::Val{outstate},
    ::Val{instate},
) where {dim, N, outstate, instate, nvertelem}
    FT = eltype(P)
