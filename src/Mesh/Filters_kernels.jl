                ...
                @loop for j in (1:Nq; threadIdx().y)
                    @loop for i in (1:Nq; threadIdx().x)
                        @unroll for n in 1:Nq
                            @unroll for fs in 1:nfilterstates
                                l_Qfiltered[fs, i, j, k] +=
                                    s_filter[i, n] * s_Q[n, j, k, fs]
                            end
                        end
                    end
                end
...
function knl_apply_TMAR_filter!(
    ::Val{nreduce},
    ::Val{dim},
    ::Val{N},
    Q,
        ...
        @loop for j in (1:Nqj; threadIdx().y)
            @loop for i in (1:Nq; threadIdx().x)
                @unroll for sf in 1:nfilterstates
                    ...
                    r = qs_average > 0 ? qs_average / qs_clipped_average :
                        zero(FT)
