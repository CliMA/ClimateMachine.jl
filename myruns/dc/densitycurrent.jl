...
# # [Example - Density Current](@id EX-RTB-docs)
#
...
# This example of a density current can be classified as an initial value problem. We must (at the very least) assign values for the initial variables in a sensible manner. This example demonstrates the use of functions defined in the [`MoistThermodynamics`](@ref MoistThermodynamics-docs) package to generate the appropriate initial state for our problem.
...
"""
    init_densitycurrent!(bl, state, aux, (x,y,z), t)
...
"""
    config_densitycurrent(FT, N, resolution, xmax, ymax, zmax)
...
function config_densitycurrent(FT, N, resolution, xmax, ymax, zmax)
    ...
    config = ClimateMachine.AtmosLESConfiguration(
        "DensitycurrentSmago",   # Problem title [String]
...
function main()
    # These are essentially arguments passed to the [`config_densitycurrent`](@ref config-helper) function.
...
# `julia --project /experiments/AtmosLES/densitycurrent.jl` will run the experiment from the main ClimateMachine directory,
