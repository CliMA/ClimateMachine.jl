...
#θ_r_vals = [0.0506, 0.0585, 0.0630,0.0686, 0.0640,0.0641, 0.0667]
#ν_vals = [0.4250,0.4182,0.4257,0.4233,0.4326,0.4344]
#log10α_vals = [-2.1905,-2.0280,-1.891,-2.0424,-2.024, -1.9778]
#log10n_vals = [0.1609, 0.1319,0.1154,0.1292, 0.1321, 0.1235]
#depth = [[37,53],[53,85],[85,105],[105,128], [128,158], [158, 200]]
#d = reverse(mean.(depth)*(-0.01))
#ν = mean(ν_vals[1:3])
#θ_r = mean(θ_r_vals[1:3])
#a_s = Spline1D(d, reverse(log10α_vals), k=1)
#function αf(z::F, f::Spline1D) where {F}
#    return F(10.0^f(z))*F(100.0)
#end

#vgα = (aux) -> αf(aux.z, as)
#n_s = Spline1D(d, reverse(log10n_vals), k=1)
#function nf(z::F, f::Spline1D) where {F}
#    return F(10.0^f(z))
#end

#vgn = (aux) -> nf(aux.z, n_s)
...
file = "data/elreno/AMF_US-ARc_BASE_HH_3-5.csv"
data = readdlm(file,',')
columns = data[3,:]
data = data[4:end, :]

# Deal with dates
tss = (data[:,columns .== "TIMESTAMP_START"])
tss = DateTime.(string.(tss), "yyyymmddHHMM")
tse = (data[:,columns .== "TIMESTAMP_END"])
tse = DateTime.(string.(tse), "yyyymmddHHMM")
# tse is always 1.5 hours ahead, only us tss
#(325 - 525 worked) - missing last layer
...
#bottom_state = (aux, t) -> eltype(aux)(ν);
...
d2005 = readdlm("data/elreno/2022_SOIL_YEAR=2005.csv",',')
soil_data = d2005[2:end,:]
columns = d2005[1,:]

date = DateTime.(soil_data[:,2])
keep = ((date .<cutoff2) .+ (date .> cutoff1)) .==2
swc = FT.(soil_data[keep,:][1,4:8]) .* 0.01
depths = FT.([-2,-4,-8,-20, -40])*2.5/100.0
...
mask = z .== depths[4]
l4 = [dons_arr[k]["soil.water.ϑ_l"][mask][1] for k in 1:N]
plot1 = plot(times,l1, label = "", color = "red", title = "Layer: -2 inches")
scatter!(date[keep], soil_data[keep,4]./100, ms = 2, color = "blue", label = "")
plot!(ylim = [0,0.4])

plot2 = plot(times,l2, label = "", color = "red", title= "Layer: - 4 inches")
scatter!(date[keep], soil_data[keep,5]./100, ms = 2, color = "blue", label = "")
plot!(ylim = [0,0.4])
plot3 = plot(times,l3, label = "", color = "red", title = "Layer: -8 inches")
scatter!(date[keep], soil_data[keep,6]./100, ms = 2, color = "blue", label = "")
plot!(ylim = [0,0.4])
plot4 = plot(times,l4, label = "", color = "red", title = "Layer: -20 inches")
scatter!(date[keep], soil_data[keep,7]./100, ms = 2, color = "blue", label = "")
plot!(ylim = [0,0.4])
