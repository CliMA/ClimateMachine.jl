...
function init_baroclinic_wave!(bl, state, aux, coords, t)
    ...
    τ_z_1::FT = exp(Γ*(r-_a)/T_0)
    τ_z_2::FT = 1 - 2*((r-_a)/b/H)^2
    τ_z_3::FT = exp(-((r-_a)/b/H)^2)
    τ_1::FT = 1/T_0 * τ_z_1 + B * τ_z_2 * τ_z_3 
    τ_2::FT = C * τ_z_2 * τ_z_3 
    τ_int_1::FT = A * (τ_z_1-1) + B * (r-_a) * τ_z_3
    τ_int_2::FT = C * (r-_a) * τ_z_3
    ...
    u_cart = SVector{3, FT}(-u_sphere[1] * sin(λ) - u_sphere[2] * sin(φ) * cos(λ), u_sphere[1] * cos(λ) - u_sphere[2] *sin(φ) * sin(λ), u_sphere[2] * cos(φ))
