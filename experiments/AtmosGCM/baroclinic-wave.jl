...
struct BaroclinicWaveRefState <: ReferenceState end
vars_state_auxiliary(::BaroclinicWaveRefState, FT) = @vars(ρ::FT, ρu::SVector{3,FT}, ρe::FT, p::FT, T::FT)

function atmos_init_aux!(
    m::BaroclinicWaveRefState,
    bl::AtmosModel,
    aux::Vars,
    geom::LocalGeometry,
)
    ...
    u_ref::FT = -_Ω*(_a + γ*z)*cos(φ) + sqrt(max(FT(0), (_Ω*(_a + γ*z)*cos(φ))^2 + (_a + γ*z)*cos(φ)*U))
    v_ref::FT = 0
    w_ref::FT = 0
...
function config_baroclinic_wave(FT, poly_order, resolution)
    ...
    ref_state = BaroclinicWaveRefState()
...
function main()
    ...
    #timeend::FT = 86400    # end time (s)
    timeend::FT = 30 * 24 * 60 * 60    # end time (s)
