...
function init_baroclinic_wave!(problem, bl, state, aux, coords, t)
    ...
    q_0::FT = 0.000                ## Maximum specific humidity (default: 0.018)
    q_t::FT = 0.000             ## Specific humidity above artificial tropopause
    ...
    if bl.moisture isa EquilMoist
      ...
      #@info @sprintf("""initial moist setup wornking :) L: %s""", LH_v0(bl.param_set) )
    end
