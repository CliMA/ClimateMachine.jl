...
function init_baroclinic_wave!(bl, state, aux, coords, t)
    ...
    _p_w::FT = 34e3          ## Pressure width parameter for specific humidity
    _η_crit::FT = 10_p_0/_p_w ## Critical pressure coordinate
    _q_0::FT = 0.018         ## Maximum specific humidity
    _q_t::FT = 1e-12         ## Specific humidity above artificial tropopause
    _φ_w::FT = 2π/9          ## Specific humidity latitude wind parameter
    ...
    if η > _η_crit
        q_tot = _q_0 * exp(-(φ/_φ_w)^4) * exp(-((η-1)*_p_0/_p_w)^2)
    else
        q_tot = _q_t
    end
    ...
    ## Moisture treated as a passive tracer 
    ## within the specific GCM test case 
...
function config_baroclinic_wave(FT, poly_order, resolution)
    ...
    model = AtmosModel{FT}(
        ...
        param_set;
        ref_state = ref_state,
        turbulence = SmagorinskyLilly{FT}(0.2),
        hyperdiffusion = BiharmonicHyperDiffusion(FT(2*3600)),
