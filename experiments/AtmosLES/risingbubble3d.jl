...
#
# 1) Dry RTB3d (circular potential temperature perturbation)
# 2) Boundaries
...
#md #       purposes)
function init_rtb3d!(bl, state, aux, (x, y, z), t)
    ## Problem float-type
    FT = eltype(state)
    ...
    if r <= r0
        Δθ = 0.5 * θamplitude * (1 + cospi(r/r0))
    end
...
# model.
function config_rtb3d(FT, N, resolution, xmax, ymax, zmax)
    ...
    model = AtmosModel{FT}(
        ...
        init_state_conservative = init_rtb3d!, # Apply the initial condition
    )
    ...
    config = ClimateMachine.AtmosLESConfiguration(
        ...
        init_rtb3d!,     # Function specifying initial condition
...
function main()
    ...
    ## [`config_rtb3d`](@ref config-helper) function.  For type
    ...
    driver_config = config_rtb3d(FT, N, resolution, xmax, ymax, zmax)
...
# `julia --project=$CLIMA_HOME experiments/AtmosLES/rtb3d.jl --vtk 1smins`
