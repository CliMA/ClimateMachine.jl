...
function init_risingbubble!(problem, bl, state, aux, localgeo, t)
    ...
    Δθ = θamplitude * am^2 * sin(pi * z / zc) / (am^2 + (x - xc)^2)
    N::FT = 0.01
    ## Compute perturbed thermodynamic state:
    θ = 300 * exp(N^2 * z / _grav) + Δθ ## potential temperature
    π_exner = FT(1) + _grav^2  * (exp(-(N^2) * z / _grav) - 1) / (c_p * 300 * N^2)             ## exner pressure
    ρ = p0 * ((π_exner)^(c_v / R_gas)) / (R_gas * θ)    ## density
    T = θ * π_exner
    #e_int = internal_energy(bl.param_set, T)
    ts = PhaseDry_ρθ(bl.param_set, ρ, θ)
    @info θ - dry_pottemp(ts), θ - virtual_pottemp(ts) 
    e_int = internal_energy(ts)
    ρu = SVector(FT(ρ * 20), FT(0), FT(0))                   ## momentum
    ## State (prognostic) variable assignment
    e_kin = FT(0.5 * 20^2)                                       ## kinetic energy
...
function config_risingbubble(
    ...
    #ode_solver = ClimateMachine.HEVISolverType(FT; linear_max_subspace_size = Int(5), nonlinear_max_iterations = 3) 
