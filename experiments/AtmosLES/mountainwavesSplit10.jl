...
function config_gravitywave(FT, N, resolution, xmin, xmax, ymax, zmax, hm, a)
    ...
    z_sponge = FT(9e3) # height at which sponge begins
    α_relax = FT(1 / 60 / 15) # sponge relaxation rate in (1/seconds)
    println("α_relax  ",α_relax)
    u_relax = SVector(FT(10), FT(0), FT(0)) # relaxation velocity
...
function main()
    ...
    cbvtk = GenericCallbacks.EveryXSimulationSteps(nvtk)  do (init=false)
        mkpath("./vtk-rtb/")
        outprefix = @sprintf("./vtk-rtb/mountainwavesSplit10_Ne%04d_mpirank%04d_step%04d",
                         Ne[1], MPI.Comm_rank(driver_config.mpicomm), vtk_step)
