...
function run(mpicomm, ArrayType, dim, topl, N, timeend, DT, dt, C_smag, LHF, SHF, C_drag, zmax, zsponge; restarting = 0)
  ...
  Data = ArrayType(localQ)
  GData =ArrayType(localvgeo)
  Q = restart_ode_state(dg, Data, GData)
  host_array = Array ∈ typeof(Q).parameters
  localQ1 = host_array ? Q.realdata : Array(Q.realdata)
  for e in 1:size(localQ,3)
    for n in 1:size(localQ,1)
      for s in 1:size(localQ,2)
        if localQ1[n,s,e] != localQ[n,s,e]
	  return 0
	end
      end
    end
...
let
  ...
  @testset "$(@__FILE__)" for ArrayType in ArrayTypes
    ...
    result1 = run(mpicomm, ArrayType, dim, topl,
                 N, timeend, DT, dt, C_smag, LHF, SHF, C_drag, zmax, zsponge; restarting=1)
    @test result1 == 1
