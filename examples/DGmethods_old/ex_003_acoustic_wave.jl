...
let
    ...
    function do_output(vtk_step)
        ...
        filename = @sprintf(
            "%s/acoustic_wave_mpirank%04d_step%04d",
            VTKDIR,
    ...
    starttime = Ref(now())
    cb_info =
        GenericCallbacks.EveryXWallTimeSeconds(60, mpicomm) do (init = false)
            if init
                starttime[] = now()
            end
            with_logger(mpi_logger) do
