...
function initialcondition!(Q, x_1, x_2, x_3, _...)
    @inbounds Q[1] =
        exp(sin(2π * x_1)) * exp(sin(2π * x_2)) * exp(sin(2π * x_3))
end
#md nothing # hide
...
let
    ...
    cb_vtk = GenericCallbacks.EveryXSimulationSteps(20) do
        vtk_step += 1
        filename = @sprintf(
            "vtk/advection_mpirank%04d_step%04d",
    ...
    starttime = Ref(now())
    cb_info =
        GenericCallbacks.EveryXWallTimeSeconds(1, mpicomm) do (init = false)
            if init
                starttime[] = now()
            else
                with_logger(mpi_logger) do
