...
@testset "Multistate RK solvers" begin
    ...
    model_fast = FastODE{FT}(Ω, ω)
    model_slow = SlowODE{FT}(Ω, ω)
        ...
        for (slow_method, slow_expected_order) in mrrk_methods
            for (fast_method, fast_expected_order) in mrrk_methods
                for (n, dt) in enumerate(dts)
                    ...
                    @show error[n] = norm(
                        [Qᶠ.data[1, 1, 1], Qˢ.data[1, 1, 1]] -
                        exactsolution(ω, finaltime),
                    )
                end
    ...
    @testset "MSMRRK methods (with substeps)" begin
        finaltime = 5π / 2
        dts = [2.0^(-k) for k in 2:12]
        error = similar(dts)
        for (slow_method, slow_expected_order) in mrrk_methods
            for (fast_method, fast_expected_order) in mrrk_methods
