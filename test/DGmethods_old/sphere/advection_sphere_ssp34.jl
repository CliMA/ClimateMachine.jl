...
function main(mpicomm, FT, topl, N, timeend, ArrayType, dt, ti_method)
    ...
    #{{{ numerical fluxex
    numflux!(x...) = NumericalFluxes.rusanov!(x..., advectionflux!, wavespeed)
    ...
    cbvtk = GenericCallbacks.EveryXSimulationSteps(1000) do (init = false)
        outprefix = @sprintf(
            "vtk/advection_sphere_%dD_mpirank%04d_step%04d",
            3,
...
function run(
    ...
    (error, Δmass) =
        main(mpicomm, FT, topl, N, timeend, ArrayType, dt, ti_method)
end
#}}} Run Script
