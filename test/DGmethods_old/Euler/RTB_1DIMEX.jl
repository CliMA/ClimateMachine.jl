...
using CLIMA.DGBalanceLawDiscretizations1D
...
# {{{ Linearization
@inline function lin_eulerflux!(F, Q, aux, t)
  F .= -zero(eltype(Q))
  @inbounds begin
    DFloat = eltype(Q)
...
@inline function lin_bcstate!(QP, auxP, nM, QM, auxM, bctype, t)
  if bctype == 1
    # this is already a linear boundary condition
    nofluxbc!(QP, nothing, auxP, nM, QM, nothing, auxM, bctype, t)
  else
    error("Unsupported boundary condition")
  end
end
...
function run(mpicomm, dim, brickrange, periodicity, N, timeend, DFloat, dt, output_steps)
  ...
  NumericalFluxes.rusanov_boundary_flux!(x..., lin_eulerflux!, lin_bcstate!,
                                         # (_...)->0) # central
                                         wavespeed_linear)
  lin_spacedisc = DGBalanceLaw1D(grid = grid,
                                 length_state_vector = _nstate,
                                 flux! = lin_eulerflux!,
                                 numerical_flux! = lin_numflux!,
                                 numerical_boundary_flux! = lin_numbcflux!,
                                 auxstate = spacedisc.auxstate,
                                 source! = lin_source!)
...
let
  ...
  for dim in (2, 3)
    ...
    domain_start = (0, 0, 0)
    domain_end = (1000, 1000, 1000)
    NeH = 10
    NeV = 100

    dt = min(NeH) / soundspeed_air(300.0) / polynomialorder^2

    output_time = 0.5
    output_steps = ceil(output_time / dt)

    Ne = dim == 2 ? (NeH, NeV, 0) : (NeH, NeH, NeV)
