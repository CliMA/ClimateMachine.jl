...
@testset "DomainIdx, all domains" begin
    dd, dss = DomainDecomp(gm = 1, en = 1, ud = 4),
...
@testset "Test global indexing, many updrafts" begin
    ...
    idx_ss_per_var =
        Dict([name => DomainIdx(dd, dss_per_var[name]) for name in var_names]...)
    a_map = Dict([name => SV.get_sv_a_map(idx, idx_ss_per_var[name]) for
    name in var_names]...)
    gm, en, ud, sd, al = allcombinations(idx)
    ...
    @test_throws BoundsError SV.var_suffix(
        vm,
        idx,
        idx_ss_per_var[:a],
        :a,
