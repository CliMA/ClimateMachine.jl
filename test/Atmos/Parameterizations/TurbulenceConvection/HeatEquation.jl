...
@testset "∂_t T = K ΔT, T = {1, 0} ∈ {z_{min}, z_{max}}, diffusion implicit" begin
    ...
    tmp = StateVec(
        ((:ΔT, dss), (:a_tau, dss), (:ρ_0, dss), (:K, dss), (:a, dss)),
        grid,
        dd,
    )
    rhs = deepcopy(q)
...
@testset "∂_t T = ∇ • (K(z) ∇T) + 1, T = {0, 0} ∈ {z_{min}, z_{max}}, comparison" begin
    ...
    tmp = StateVec(
        ((:ΔT, dss), (:a_tau, dss), (:ρ_0, dss), (:K_thermal, dss), (:a, dss)),
        grid,
        dd,
    )
    rhs = deepcopy(q)
    ...
    export_plots && plot_state(
        q,
        grid,
        heat_eq_dir,
        :T_explicit_surf,
        filename = "T_varK_explicit_surf.png",
    )
    export_plots && plot_state(
        q,
        grid,
        heat_eq_dir,
        :T_explicit_vol,
        filename = "T_varK_explicit_vol.png",
    )
    export_plots && plot_state(
        q,
        grid,
...
@testset "∂_t T = ∇ • (K(z) ∇T) + VS, -K(z) ∇T = 1 ∈ z_{min}, T = 0 ∈ z_{max}, comparison" begin
    ...
    tmp = StateVec(
        ((:ΔT, dss), (:a_tau, dss), (:ρ_0, dss), (:K_thermal, dss)),
        grid,
        dd,
    )
    rhs = deepcopy(q)
    ...
    export_plots && plot_state(
        ...
        heat_eq_dir,
        :T_explicit_surf,
        filename = "T_cosK_explicit_surf.png",
    )
    export_plots && plot_state(
        q,
        grid,
        heat_eq_dir,
        :T_explicit_vol,
        filename = "T_cosK_explicit_vol.png",
    )
    export_plots && plot_state(
        q,
        grid,
