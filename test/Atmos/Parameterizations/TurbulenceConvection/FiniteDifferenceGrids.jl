...
@testset "Grid interface" begin
    for FT in (Float32, Float64)
        for n_ghost in (1, 2)
            ...
            @test over_elems_ghost(grid) ==
                  [(1:n_ghost)..., ((n_elems + 1 - n_ghost):n_elems)...]
...
@testset "Grid operators convergence" begin
    ...
    for n_elems_real in [2^k for k in 3:8]
        ...
        del_err = [
            abs(
                ∇ϕ[k] -
                ∇_z_upwind(ϕ[(k - 1):(k + 1)], w[(k - 1):(k + 1)], grid),
            ) for k in over_elems_real(grid)
        ]
        @test all([e < tol * grid.Δz for e in del_err])

        del_err = [
            abs((∇ϕ[k - 1] + ∇ϕ[k]) / 2 - ∇_z_flux(ϕ[(k - 1):k], grid))
            for k in over_elems_real(grid)
        ]
        @test all([e < tol * grid.Δz^2 for e in del_err])
        del_err = [
            abs((∇ϕ[k] + ∇ϕ[k + 1]) / 2 - ∇_z_flux(ϕ[k:(k + 1)], grid))
            for k in over_elems_real(grid)
        ]
        @test all([e < tol * grid.Δz^2 for e in del_err])

        del_err = [
            abs(∇ϕ[k] - ∇_z_centered(ϕ[(k - 1):(k + 1)], grid))
            for k in over_elems_real(grid)
        ]
        @test all([e < tol * grid.Δz^2 for e in del_err])
        ...
        del_err = [
            abs(
                (∇ϕ[k - 1] + ∇ϕ[k]) / 2 - ∇_z_dual(ϕ[(k - 1):(k + 1)], grid)[1],
            ) for k in over_elems_real(grid)
        ]
        @test all([e < tol * grid.Δz^2 for e in del_err])
        del_err = [
            abs(
                (∇ϕ[k] + ∇ϕ[k + 1]) / 2 - ∇_z_dual(ϕ[(k - 1):(k + 1)], grid)[2],
            ) for k in over_elems_real(grid)
        ]
        @test all([e < tol * grid.Δz^2 for e in del_err])

        Lap_err = [
            abs(∇ψ[k] - Δ_z(ϕ[(k - 1):(k + 1)], grid))
            for k in over_elems_real(grid)
        ]
        @test all([e < tol * grid.Δz^2 for e in Lap_err])

        Lap_err = [
            abs(∇K∇ϕ[k] - Δ_z(ϕ[(k - 1):(k + 1)], grid, K[(k - 1):(k + 1)])) for k in over_elems_real(grid)
        ]
