...
@testset "BGMRES" begin
    @kernel function mul_by_A!(x, A, y, n1, n2)
        I = @index(Global)
        for i in 1:n1
            tmp = zero(eltype(x))
            for j in 1:n2
        ...
        for ArrayType in Arrays
            for T in [Float32, Float64]
                ...
                @testset "($ArrayType, $T) MPIStateArray Test" begin
                    ...
                    bgmres = IterativeSolver(
                        BatchedGeneralizedMinimalResidualAlgorithm(
                            atol = ϵ, rtol = ϵ, M = n1 * n2, dims = size(mpi_x), batchdimindices = (1,2)
                        ),
                        mpi_x, linear_operator!, mpi_b
                    )

                    bgmres(mpi_x, linear_operator!, mpi_b)
                ...
                @testset "(Array, $T) Columnwise Test" begin
                    ...
                    bgmres = IterativeSolver(
                        BatchedGeneralizedMinimalResidualAlgorithm(atol = 10ϵ, rtol = 10ϵ, M = tup[3] * tup[5], batchdimindices = (2,)),
                        x, columnwise_linear_operator!, b
                    )
