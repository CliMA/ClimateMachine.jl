...
struct BaroclinicWave <: AbstractMoistAtmosProblem end
...
function run(
    ...
    model = MoistAtmosModel{FT}(
        SphericalOrientation(),
        problem,
        ref_state = MoistReferenceState(T_profile),
        sources = sources,
    )
    ...
    linearmodel = MoistAtmosAcousticGravityLinearModel(model)
    lineardg = DGModel(
        linearmodel,
        grid,
