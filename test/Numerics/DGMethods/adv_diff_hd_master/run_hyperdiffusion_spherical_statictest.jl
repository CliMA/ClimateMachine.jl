# run from ClimateMachine.jl

# Specify modules used
using MPI
using ClimateMachine
using Logging
using ClimateMachine.Mesh.Topologies
using ClimateMachine.Mesh.Grids
using ClimateMachine.DGMethods
using ClimateMachine.DGMethods.NumericalFluxes
using ClimateMachine.MPIStateArrays
using LinearAlgebra
using Printf
using Dates
using ClimateMachine.GenericCallbacks:
    EveryXWallTimeSeconds, EveryXSimulationSteps
using ClimateMachine.ODESolvers
using ClimateMachine.VTK: writevtk, writepvtu
using ClimateMachine.Mesh.Grids: min_node_distance
using ClimateMachine.Atmos: SphericalOrientation, latitude, longitude

using ClimateMachine.Orientations
using CLIMAParameters
using CLIMAParameters.Planet
using GLMakie

# Import and update Earth parameters
struct EarthParameterSet <: AbstractEarthParameterSet end
const param_set = EarthParameterSet()
# modify Earth radius in the CLIMAParameter
CLIMAParameters.Planet.planet_radius(::EarthParameterSet) = 60e3 

# Include helper functions for this particular experiment
include("hyperdiffusion_model.jl")
include("run_function.jl")
include("initial_condition.jl")
include("spherical_harmonics_kernels.jl")
include("output_writer.jl")
include("makie_gui.jl")
include("grid.jl")

# Initialise CM
ClimateMachine.init()
ArrayType = ClimateMachine.array_type()
mpicomm = MPI.COMM_WORLD

# Choose output directory for vtk files
output_dir = "output"

# Set global model parameters
direction = HorizontalDirection # direction for diffusion kernels
dim = 3                         # no dimensions
_a = planet_radius(param_set)   # planet radius (m)
height = 30.0e3 #_a * 0.01      # domain height (m)
FT = Float64 
polynomialorder = 5
horz_num_elem = 8
vert_num_elem = 4
τ = 1                            # timescale for hyperdiffusion

# Define topology (StackedCubedSphereTopology, StackedBrickTopology)
topl = StackedCubedSphereTopology(
    mpicomm,
    horz_num_elem,
    grid1d(_a, _a + height, nelem = vert_num_elem)
)
# Call the DG grid specific to the topology
grid = get_grid(FT, topl, ArrayType, polynomialorder)

# Choose which model (balance law) to run
model = HyperDiffusion{dim}(ConstantHyperDiffusion{dim, direction(), FT}(D, l, m))

# run model
dg, rhs_DGsource, rhs_anal, rel_error = run(mpicomm, ArrayType, dim, topl, grid, model, polynomialorder, FT, direction, τ*3600, 7, 4 )

# get vtk output- plot with paraview
do_output_vtk(mpicomm, output_dir, dg, rhs_DGsource, rhs_anal, model)

# plot output with makie

#makie_gui(topl, grid, rhs_DGsource, rhs_anal)# this wont work until this spherical topology is supported

nothing



