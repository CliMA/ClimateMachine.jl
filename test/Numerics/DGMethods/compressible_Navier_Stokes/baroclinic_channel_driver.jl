...
function config_baroclinicwave(FT, N, resolution, xmax, ymax, zmax)
    ...
    problem = AtmosProblem(
        boundaryconditions = (
            InitStateBC(),
            InitStateBC(), #Here if you set the second tuple entry to [AtmosBC()] this defaults to freeslip
        ),
    ...
    model = AtmosModel{FT}(
        ...
        turbulence = SmagorinskyLilly(FT(0)),
...
function config_diagnostics(driver_config, FT, xmax, ymax, zmax)
    ...
    resolution = (FT(200e3), FT(200e3), FT(1000)) 
