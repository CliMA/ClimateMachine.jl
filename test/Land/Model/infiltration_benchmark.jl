...
# (Q, f) -> JacobianFreeNewtonKrylovSolver(Q, BatchedGeneralizedMinimalResidual(
#     f,
#     Q;
...
# Newton + (B)GMRES
algs = (
    N_FD = (Q, f) -> JacobianFreeNewtonKrylovAlgorithm(
    ...
    N_AD = (Q, f) -> JacobianFreeNewtonKrylovAlgorithm(
                GeneralizedMinimalResidualAlgorithm(;
                    preconditioner = ColumnwiseLUPreconditioner(f, Q, 100),
                    M = 30, atol = eps(Float64), rtol = 1e-9,
                );
            atol = 1e-9, rtol = 1e-9, autodiff=true),

    P = (Q, f) -> StandardPicardAlgorithm(; atol = 1e-9, rtol = 1e-9, maxiters = 1000),
...
for i in 1:4
    solver_config = setupsolver(algs[i], driver_config, 0.0, 1.0, 1.0)
...
dts = FT[0.05, 0.1, 0.2, 0.5, 1, 2, 5, 10]
...
timeplot = plot(dts, [solvetimes[i, :] for i in 1:N],
    title="Solve Times", xlabel="dt", ylabel="total solve time", markershape=:circle, label = labels)
png(timeplot, joinpath(@__DIR__, "solvetimes"))
errplot = plot(dts, [rmse[i, :] for i in 1:N],
    title="RMSE", xlabel="dt", ylabel="rmse", markershape=:circle, legend=:topleft, label = labels)
png(errplot, joinpath(@__DIR__, "rmse"))
@save joinpath(@__DIR__, "benchmark_data.jld2") solvetimes rmse dts
