...
%
ddataz = load('dycoms_Z.dat');
size(ddataz)

dthetal = load('dycoms_THL.dat');
size(dthetal)

ddatatempe = load('dycoms_T.dat');
size(ddatatempe)

ddatap = load('dycoms_P.dat');
size(ddatap)

ddataq = load('dycoms_qt.dat');
size(ddataq)


sounding(:, 1) = ddataz';
sounding(:, 2) = dthetal';
sounding(:, 3) = 1e+3*ddataq';
sounding(:, 4) = ug;
sounding(:, 5) = vg;
sounding(:, 6) = ddatap';
...
%write to file
fileID = fopen('sounding_DYCOMS_from_PyCles.dat','w');
