...
function run(mpicomm, ArrayType, LinearType,
  ...
  mis = MIS2(slow_dg, (fast_dg_h, fast_dg_v), (dgt,Q) -> MultirateRungeKutta(:LSRK144NiegemannDiehlBusch, dgt, Q, nsteps=ns2), ns1, Q; dt = dt, t0 = 0)
  ...
  step = [0]
  cbvtk = GenericCallbacks.EveryXSimulationSteps(4)  do (init=false)
    mkpath("./vtk-rtb/")
      outprefix = @sprintf("./vtk-rtb/DC_%dD_mpirankSplitMultirateRK%04d_step%04d", dim,
