...
function run(mpicomm, ArrayType, dim, topl, N, timeend, FT, dt, C_smag, LHF, SHF, C_drag, zmax, zsponge, VTKPATH)
  ...
  step = [0]
    cbvtk = GenericCallbacks.EveryXSimulationSteps(900000000) do (init=false)
    mkpath(VTKPATH)
  ...
  #Extract statistics:
  cbdiags = GenericCallbacks.EveryXSimulationSteps(1000) do (init=false)
    gather_diags(dg, Q)
  end
    
  solve!(Q, lsrk; timeend=timeend, callbacks=(cbdiags, cbinfo, cbvtk))

  #Extract statistics at last time step:
  gather_diags(dg, Q)
