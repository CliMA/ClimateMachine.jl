...
function init_state!(state::Vars, aux::Vars, (x1,x2,x3), t, spl_tinit, spl_qinit, spl_uinit, spl_vinit, spl_pinit)
  DFloat         = eltype(state)
  ...
  state.moisture.ρq_liq = DFloat(0)
  state.moisture.ρq_ice = DFloat(0)
  state.precipitation.ρq_rain = DFloat(0)
  nothing
end
...
function run(mpicomm, ArrayType, dim, topl, N, timeend, DFloat, dt)
  ...
  model = AtmosModel(FlatOrientation(),
                     ConstantViscosityWithDivergence(DFloat(1/100)),
                     NonEquilMoist(),
                     Rain(),
                     NoRadiation(),
                     source!,
  ...
  Q = init_ode_state(dg, param, DFloat(0), spline_int())

  mkpath("vtk")
  mkpath(joinpath("vtk","squal_line"))
  outprefix = "vtk/squal_line/state_init_$(dim)D_mpirank$(MPI.Comm_rank(mpicomm))_step$(DFloat(0))"
  writevtk(outprefix, Q, dg, flattenednames(vars_state(model, DFloat)))

  outprefix = "vtk/squal_line/aux_init_$(dim)D_mpirank$(MPI.Comm_rank(mpicomm))_step$(DFloat(0))"
  ...
  cbinfo = GenericCallbacks.EveryXSimulationSteps(1) do (s=false)
    if s
      ...
      outprefix = "vtk/squal_line/state_$(dim)D_mpirank$(MPI.Comm_rank(mpicomm))_step$(DFloat(ODESolvers.gettime(lsrk)))"
      writevtk(outprefix, Q, dg, flattenednames(vars_state(model, DFloat)))
  ...
  engf = norm(Q)
  Qe = init_ode_state(dg, param, DFloat(timeend), spline_int())
  ...
  n_runs = 2
    for DFloat in (Float64,) #Float32)
      ...
      for nth_run in 1:n_runs
        ...
        brickrange = (range(DFloat(xmin); length=Ne[1]+1, stop=DFloat(xmax)),
                      range(DFloat(ymin); length=Ne[2]+1, stop=DFloat(ymax)),
                      range(DFloat(zmin); length=Ne[2]+1, stop=DFloat(zmax)))
        ...
        result_n = run(mpicomm, ArrayType, dim, topl, polynomialorder, timeend, DFloat, dt)
      ...
      @show results_diff
      println("------------------------------------------------------------")
      @test all([x<eps(DFloat) for x in results_diff])
    end
  end
end
