...
function init_velocity_diffusion!(
    ...
    uλ = 2 * FT(π) * cos(φ) * r
    uφ = 0
    aux.u  = SVector(-uλ * sin(λ) - uφ * cos(λ) * sin(φ),
...
function update_velocity_diffusion!(::ReversingDeformationalFlow,
                                    ::AdvectionDiffusion, state::Vars, aux::Vars, t::Real)
    FT = eltype(aux)
    λ = longitude(SphericalOrientation(), aux)
    φ = latitude(SphericalOrientation(), aux)
    r = norm(aux.coord)
    T = FT(5)
    λp = λ - FT(2π) * t / T
    uλ = 10 * r / T * sin(λp) ^ 2 * sin(2φ) * cos(FT(π) * t / T) + FT(2π) * r / T * cos(φ)
...
function boundary_state!(::Rusanov, ::AdvectionDiffusion, stateP::Vars, auxP::Vars,
...
let
    ...
    expected_result[SolidBodyRotation, LSRK144NiegemannDiehlBusch, 1] = 1.3199024557832748e-01
    expected_result[SolidBodyRotation, LSRK144NiegemannDiehlBusch, 2] = 1.9868931633120656e-02
    expected_result[SolidBodyRotation, LSRK144NiegemannDiehlBusch, 3] = 1.4052110916915061e-03
    expected_result[SolidBodyRotation, LSRK144NiegemannDiehlBusch, 4] = 9.0193766298676310e-05
    
    expected_result[SolidBodyRotation, SSPRK33ShuOsher, 1] = 1.1055145388897809e-01
    expected_result[SolidBodyRotation, SSPRK33ShuOsher, 2] = 1.5510740467668628e-02
    expected_result[SolidBodyRotation, SSPRK33ShuOsher, 3] = 1.8629481690361454e-03
    expected_result[SolidBodyRotation, SSPRK33ShuOsher, 4] = 2.3567040048588889e-04
    
    expected_result[SolidBodyRotation, SSPRK34SpiteriRuuth, 1] = 1.2641959922001456e-01
    expected_result[SolidBodyRotation, SSPRK34SpiteriRuuth, 2] = 2.2780375948714751e-02
    expected_result[SolidBodyRotation, SSPRK34SpiteriRuuth, 3] = 3.1274951764826459e-03
    expected_result[SolidBodyRotation, SSPRK34SpiteriRuuth, 4] = 3.9734060514021565e-04
    
    expected_result[ReversingDeformationalFlow, LSRK144NiegemannDiehlBusch, 1] = 5.5387951598735408e-01
    expected_result[ReversingDeformationalFlow, LSRK144NiegemannDiehlBusch, 2] = 3.7610388138383732e-01
    expected_result[ReversingDeformationalFlow, LSRK144NiegemannDiehlBusch, 3] = 1.7823508719111605e-01
    expected_result[ReversingDeformationalFlow, LSRK144NiegemannDiehlBusch, 4] = 3.8639493470255713e-02
    
    expected_result[ReversingDeformationalFlow, SSPRK33ShuOsher, 1] = 5.5353962032596349e-01
    expected_result[ReversingDeformationalFlow, SSPRK33ShuOsher, 2] = 3.7645487928038762e-01
    expected_result[ReversingDeformationalFlow, SSPRK33ShuOsher, 3] = 1.7823263736245307e-01
    expected_result[ReversingDeformationalFlow, SSPRK33ShuOsher, 4] = 3.8605903366230925e-02
    
    expected_result[ReversingDeformationalFlow, SSPRK34SpiteriRuuth, 1] = 5.5404045660832824e-01
    expected_result[ReversingDeformationalFlow, SSPRK34SpiteriRuuth, 2] = 3.7788858038003154e-01
    expected_result[ReversingDeformationalFlow, SSPRK34SpiteriRuuth, 3] = 1.8007113931230376e-01
    expected_result[ReversingDeformationalFlow, SSPRK34SpiteriRuuth, 4] = 3.9941331660544775e-02
    
    numlevels = integration_testing || CLIMA.Settings.integration_testing ? 4 : 1
          ...
          for problem in (SolidBodyRotation, ReversingDeformationalFlow)
            ...
            for l in 1:numlevels
                ...
                topl = StackedCubedSphereTopology(
                    mpicomm,
                    numelems_horizontal,
                    range(FT(1), stop = 2, length = numelems_vertical + 1),
                )
