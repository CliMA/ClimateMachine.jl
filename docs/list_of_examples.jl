...
examples_files = [joinpath(root, f) for (root, dirs, files) in Base.Filesystem.walkdir(examples_dir) for f in files]
...
examples = [joinpath(root, f) for (root, dirs, files) in Base.Filesystem.walkdir(generated_dir) for f in files]
examples = map(x-> last(split(x, joinpath("docs","src","generated"))), examples)
# @show examples
